...
| Pair (Path A ↔ Path B) | Score (Large) | Score (Small) | Comment |
| :--- | ---: | ---: | :--- |
| `lead to` ↔ `result in` | **0.1536** | 0.0000 | Correct causal paraphrase (“X leads to Y” ≈ “X results in Y”). |
| `die from` ↔ `die of` | **0.1309** | 0.0000 | Correct preposition alternation in medical contexts. |
| `protect against` ↔ `protect from` | **0.1075** | 0.0000 | Same predicate with near-interchangeable prepositions. |
| `consist of` ↔ `contain` | **0.0983** | 0.0000 | Part–whole relation that often functions synonymously in context. |
| `affect` ↔ `attack` | 0.0285 | **0.0285** | **Only TP found in both runs**; very frequent verbs survive sparsity. |

### 4.2 False Positives (System: Yes, Truth: No)
| Pair (Path A ↔ Path B) | Score (Large) | Score (Small) | Why it fooled DIRT |
| :--- | ---: | ---: | :--- |
| `contract` (dobj) ↔ `die of` | 0.0655 | 0.0000 | **Relatedness vs. synonymy:** share the same disease nouns (cause→outcome). |
| `die of` ↔ `suffer from` | 0.0580 | 0.0000 | Same medical noun sets, but meanings differ (terminal outcome vs. condition). |
| `contract` (dobj) ↔ `die from` | 0.0418 | 0.0000 | Same pattern as above: overlapping fillers imply similarity. |
| `avoid in` ↔ `use in` | 0.0331 | 0.0000 | **Antonym/context overlap:** opposites often occur with identical contexts. |
| `die of` ↔ `get` (dobj) | 0.0264 | 0.0000 | Very broad verb (“get”) overlaps many contexts → noisy similarity. |
